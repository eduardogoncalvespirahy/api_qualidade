...
export class FormGroupsController {
  private readonly service: FormGroupsService;

  constructor() {
    this.service = new FormGroupsService();
  }
  ...
  create = async (req: Request, res: Response): Promise<Response> => {
    try {
      ...
      return res.status(400).json({
        message:
          error instanceof Error ? error.message : "Erro ao criar FormGroups",
      });
    }
  };
