...
import { SignatureFileService } from "../services/signatureFile.service";
import { CreateSignatureFileDTO } from "../models/signatureFile.model";

export class SignatureFileController {
  private readonly service: SignatureFileService;

  constructor() {
    this.service = new SignatureFileService();
  }

  // POST /files — recebe o arquivo via multer e monta o DTO
  create = async (req: Request, res: Response): Promise<Response> => {
    try {
      const dto: CreateSignatureFileDTO = {
      ...
      return res.status(400).json({
        message: error instanceof Error ? error.message : "Erro ao criar arquivo",
      });
    }
  };
  ...
  findById = async (req: Request, res: Response): Promise<Response> => {
    try {
      ...
      return res.status(404).json({
        message: error instanceof Error ? error.message : "Arquivo não encontrado",
      });
    }
  };
  ...
  update = async (req: Request, res: Response): Promise<Response> => {
    try {
      ...
      return res.status(400).json({
        message: error instanceof Error ? error.message : "Erro ao atualizar arquivo",
      });
    }
  };
